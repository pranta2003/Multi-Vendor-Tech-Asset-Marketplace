...
const log = (msg: string) => console.log(`[Marketplace Seed] ${msg}`);

export async function seedMarketplaceData(prisma: PrismaClient): Promise<number> {
...
- Production codebase refactoring case studies`,
  ...
  for (const item of demoProducts) {
    ...
    if (!vendorId || !categoryId) {
      throw new Error(`Missing vendor or category for product: ${item.slug}`);
    }
  ...
  log(`Marketplace demo seeding complete. ${demoProducts.length} items verified.`);
